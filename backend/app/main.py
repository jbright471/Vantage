...
from backend.app.services.demo import demo_mode_enabled, seed_demo_data
...
from backend.app.workers.report_scheduler import (
    report_scheduler_enabled,
    report_scheduler_worker,
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    report_scheduler_task: asyncio.Task[None] | None = None
    if background_polling_enabled():
        ...
        if report_scheduler_enabled():
            report_scheduler_stop_event = asyncio.Event()
            app.state.report_scheduler_stop_event = report_scheduler_stop_event
            report_scheduler_task = asyncio.create_task(report_scheduler_worker(report_scheduler_stop_event, config))
            app.state.report_scheduler_task = report_scheduler_task

    try:
        yield
    finally:
        if report_scheduler_task is not None:
            await stop_report_scheduler_task(report_scheduler_task, app.state.report_scheduler_stop_event)
